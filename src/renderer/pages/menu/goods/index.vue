  ...
  <div>
    <Form :label-width="90"
      ...
      <FormItem :label="item.label"
                v-for="(item,index) in searchInputNumberType"
                :key="index">
        <InputNumber v-model="search[item.min]"
                     :min="0"
                     :max="search[item.max] ? search[item.max] :Infinity"></InputNumber>
        —
        <InputNumber v-model="search[item.max]"
                     :min="search[item.min] ? search[item.min] :0"></InputNumber>
        <Button type="text"
                shape="circle"
                icon="close-round"
                size="small"
    ...
    <Modal v-model="modalShow"
      ...
      <div>
        <Form ref="formVali"
          ...
          <FormItem label="标准进价"
                    prop="standard_buy_unit_price">
            <Input v-model.number="modalParams.standard_buy_unit_price"
                   placeholder="非必填，小数位不超过2位的正整数"
                   style="width: 250px"></Input>
          </FormItem>
          <FormItem label="标准售价"
                    prop="standard_sell_unit_price">
            <Input v-model.number="modalParams.standard_sell_unit_price"
                   placeholder="非必填，小数位不超过2位的正整数"
                   style="width: 250px"></Input>
          </FormItem>
          <FormItem label="备注"
